...
class Home extends Component {
  ...
  handleAddRating = async (e, id) => {
    // Write your code here
    // e.preventDefault()
    const response = await fetch(`http://localhost:8001/courses/rating/${id}`, { method : 'PATCH',
  ...
  handleDrop = async (e, id) => {
    // Write your code here
    // e.preventDefault()
    const response = await fetch(`http://localhost:8001/courses/drop/${id}`, { method : 'DELETE', headers: { "Content-Type": "application/json" }});
    const val = await response.json();
    if( val.message) {
      this.setState({show : true})
    }
          ...
          return(
              ...
              <div className="card" key={val._id}>
                <ul>
                  ...
                  <div className="footer">
                    <li> {`${val.duration} hrs . ${val.noOfRatings} Ratings . ${val.rating}/5`}</li>
                  </div>
                </ul>
              </div>
